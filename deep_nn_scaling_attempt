...
def deep_nn(X, y, hidden_sizes, num_epochs=100, learning_rate=0.0001, random_state=12345, funct=None) :
    if random_state is not None:
        rng = np.random.default_rng(random_state)
    else:
        rng = np.random.default_rng()
    w = []
    w.append(rng.normal(scale=1/X[0].size, size=(X[0].size, hidden_sizes[0])))
    if len(hidden_sizes) > 1:
        for i in range(len(hidden_sizes)-1):
            w.append(rng.normal(scale=1/hidden_sizes[i], size=(hidden_sizes[i], hidden_sizes[i+1])))
    w.append(rng.normal(scale=1/np.sqrt(hidden_sizes[-1]), size=(hidden_sizes[-1], y[0].size)))
    b = []
    for i in range(len(hidden_sizes)):
        b.append(hidden_sizes[i])
    b.append(np.zeros(y[0].size))
    ...
    for epoch in range(num_epochs):
        ...
        for i in stochastic_order:
            ...
            for j in range(len(hidden_sizes)+1):
                outputs.append(forward(outputs[j], w[j], b[j], funct))
            ...
            errors.append(y[i] - outputs[-1])
            for j in range(len(hidden_sizes)+1):
                gradient, d_bias, error = backward(errors[j], outputs[-(j+1)], outputs[-(j+2)], w[-(j+1)], funct)
            ...
            for j in range(len(w)):
                w[j] = w[j] + (learning_rate * gradients[-(j+1)])
            for j in range(len(b)):
                b[j] = b[j] + (learning_rate * d_biases[-(j+1)])
...
w, b = deep_nn(train_X, train_Y, (100, 100), funct="ReLU")

z1 = forward(test_X, w[0], b[0], "ReLU")
print(z1)
z2 = forward(z1, w[1], b[1], "ReLU")
results = forward(z2, w[2], b[2], "ReLU")
